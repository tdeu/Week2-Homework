...
describe("Ballot", async () => {
  ...
  describe("when the chairperson interacts with the giveRightToVote function in the contract", async () => {
    it("gives right to vote for another address", async () => {
      ...
      const otherAccountAddress = otherAccount.account.address.toLowerCase() as `0x${string}`;
      await ballotContract.write.giveRightToVote([otherAccountAddress], {
        account: deployer.account.address,
      });
      const otherVoter = await ballotContract.read.voters([otherAccountAddress]);
      expect(otherVoter[0]).to.eq(1n); // The weight should be set to 1
    ...
    it("can not give right to vote for someone that has already voting rights", async () => {
      ...
      await expect(
        ballotContract.write.giveRightToVote([otherAccountAddress], {
          account: deployer.account.address,
        })
      ).to.be.rejected;
    });
  ...
  describe("when the voter interacts with the vote function in the contract", async () => {
    ...
    it("should register the vote", async () => {
      ...
      expect(proposal[1]).to.eq(1n); // Assuming the first proposal is voted for once
    });
  });
